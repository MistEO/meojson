...
decltype(auto) json::value::get(const std::string& key,
                                Type               default_value) const {
  return is_object() ? as_object().get(key, default_value) : default_value;
}

template <typename Type>
decltype(auto) json::value::get(size_t pos, Type default_value) const {
...
const std::string json::value::format(std::string shift_str,
